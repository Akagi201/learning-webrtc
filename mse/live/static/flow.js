...
FlowCodec = function () {
    ...
    self.toAnnexb = function (tag) {
        ...
        var sample = {
            ok: false, frame_type: frame_type, size: tag.tag.byteLength,
    ...
    self.avc_transmux_sample = function (sample) {
        ...
        // aud.
        frame.set(fresh_nalu_header, frameSize);
        frameSize += 4;
        frame.set(aud_nalu_7, frameSize);
        frameSize += 2;
        ...
        if (sample.has_idr) {
            ...
            if (self.avc.sps.byteLength > 0) {
                ...
                frameSize += 4;
                // sps
                frame.set(self.avc.sps, frameSize);
                frameSize += self.avc.sps.byteLength;
            }
            // cont nalu header before pps.
            if (self.avc.pps.byteLength > 0) {
                // AnnexB prefix, for pps always 3 bytes header
                frame.set(cont_nalu_header, frameSize);
                frameSize += 3;
                // pps
                frame.set(self.avc.pps, frameSize);
                frameSize += self.avc.pps.byteLength;
            }
        }
        ...
        for (var i in sample.nalus) {
            ...
            frameSize += 3;
            // sample data
            frame.set(nalu, frameSize);
            frameSize += nalu.byteLength;
        }
